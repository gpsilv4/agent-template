...
- **Scripts de verificacao** (`.agent/scripts/*.mjs`): doc guards, contadores do backlog, tamanhos de bundle, superficie de teste, a varredura de mutacao que prova que as suites afirmam algo, e **dois simuladores** — um monta um projeto derivado do zero, outro atualiza um que ja existia (`/upgrade`). So precisam de `node` — sem dependencias, sem `package.json` — logo qualquer agente os corre e o CI corre-os para todos. **Cada verificador tem a sua suite de testes negativos e a sua entrada em `PARES`**; a varredura reprova quem nao tiver. As suites vivem em **`.agent/scripts/tests/`** (e os construtores de fixture em `tests/harness/`), para a raiz ser so o que se invoca. Correr antes de commit; os `test-*` correm sempre no CI.
