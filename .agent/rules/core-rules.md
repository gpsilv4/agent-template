...
- **Guard Tests** (`.agent/scripts/test-guards.mjs`): testes **negativos** dos doc guards — para cada guard, quebra o que ele promete verificar e afirma que avisa e sai `!= 0`. Sem dependencias nem `package.json`. Correr apos qualquer alteracao a `check-doc-versions.mjs`: um guard que passa quando devia falhar produz confianca infundada.
