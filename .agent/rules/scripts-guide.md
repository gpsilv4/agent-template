...
- **Hooks** (`.claude/hooks/`, **so-Claude Code**): ponteiros finos que chamam as verificacoes **antes** da ferramenta correr — o agente pode esquecer uma regra do `CLAUDE.md`, um hook nao esquece. Saem `0` em tudo excepto na negacao explicita: um hook avariado nunca bloqueia trabalho legitimo. Testes em `.claude/hooks/tests/`.
