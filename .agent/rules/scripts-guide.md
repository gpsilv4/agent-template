...
- **O que SAIU do template** (`lib/upgrade-mecanico.mjs`): o `/upgrade` copia com `cpSync`, que acrescenta e substitui mas **nunca apaga** — um ficheiro renomeado no template ficava no consumidor ao lado do novo, e a descoberta exigia-lhe par (`SEM PAR`). O motor devolve agora a lista dos que **estavam na tag**, **ja nao estao no template** e **ainda existem no consumidor**; as tres condicoes juntas sao o que impede propor apagar trabalho do projeto. **Lista, nao apaga** — apagar e decisao do passo de aprovacao.
- **Guard 19 — isolamento das suites** (`.agent/scripts/guards/isolamento.mjs`): verifica o que torna a varredura **paralela** segura — cada suite monta o seu tmpdir com `mkdtempSync` (nunca um caminho fixo), nenhuma abre portas, nenhuma escreve em `process.env`. Era verdade por acidente e nada a verificava: uma suite futura com caminho fixo nao daria erro, daria **vermelhos intermitentes**, e a licao seria "volta a correr". E uma verificacao ESTATICA — prova que os tres mecanismos conhecidos nao estao escritos, nao que duas corridas nunca se pisam. Testes: `tests-isolamento.mjs`.
