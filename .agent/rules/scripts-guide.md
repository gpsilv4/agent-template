...
- **Suites de testes negativos** (`test-guards.mjs`, `test-bundle-sizes.mjs`, `test-backlog.mjs`, `test-mutation-sweep.mjs`, `test-test-surface.mjs`, `test-registo.mjs`, `test-simulate-derived.mjs`, `test-simulate-upgrade.mjs`, `test-commit-msg.mjs` e `.claude/hooks/tests/test-hooks.mjs`): uma por verificador — quebram o que ele promete verificar e afirmam que avisa e sai `!= 0`. Correm **sempre** no job `guard-tests` do CI (nao estao condicionadas ao `package.json`) e uma suite vermelha **reprova o job**. Sem dependencias nem `package.json`. Correr apos qualquer alteracao a um `check-*.mjs`: um guard que passa quando devia falhar produz confianca infundada.
