...
| **Git hook versionado** (`.githooks/X`) | suite `test-X.mjs` em `.agent/scripts/`; entrada em `PARES` no `mutation-sweep.mjs`; passo no job `guard-tests` do `ci.yml`; regra no mapa `lib/mapa-suites.mjs` (o `stop-verify.mjs` importa-o, e o `mutation-sweep --diff` tambem); passo `git config core.hooksPath .githooks` em `/setup` + `CONTRIBUTING.md`; arvore do `README.md` |
| **Modulo partilhado** (`.agent/scripts/lib/X.mjs`) | suite `test-X.mjs`; entrada em `PARES`; a descoberta do `mutation-sweep` ja varre `lib/`, logo **sem par o gate reprova** (excepto um modulo so de dados, sem um unico sitio de recusa); passo no `ci.yml`; regra no mapa `lib/mapa-suites.mjs`; entrada no `allow` de `.claude/settings.json` se for para correr |
