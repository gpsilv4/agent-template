...
| `.claude/hooks/*` + a chave `hooks` do `settings.json` | Copia se ausentes, **e adaptar `PROTEGIDOS`** aos branches deste projeto. Trazer tambem `.claude/hooks/tests/` — um hook sem testes bloqueia trabalho legitimo em silencio | **So-Claude Code.** A verificacao equivalente vive em `.agent/scripts/check-*.mjs`, que qualquer agente corre |
