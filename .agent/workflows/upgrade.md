...
| `.agent/rules/` acumuladas (`anti-patterns`) | **Acrescentar** entradas novas; nunca substituir o ficheiro. **E reescrever as citacoes `APn` que vierem nos scripts e nos workflows**: os numeros do template nao sao os deste projeto, logo uma citacao trazida de fora aponta para uma entrada **real com outro significado** — pior do que uma referencia morta, porque nada no ecra a denuncia. O Guard 15 apanha as que nao resolvem; as que resolvem **para a entrada errada** so se apanham a ler. Medido num consumidor: mais de vinte citacoes de uma vez | O projeto tem anti-padroes proprios, derivados dos seus bugs |
...
| `.claude/hooks/*` + a chave `hooks` do `settings.json` | Copia se ausentes, **e adaptar** `PROTEGIDOS` (branches deste projeto), `SUITES` no `stop-verify` (caminho -> comando que o verifica) e os verbos que o projeto tenha acrescentado a `SEGUROS` no `guard-protected-branch` (a mensagem de negacao convida a isso). Trazer tambem `.claude/hooks/tests/` **inteiro**, incluindo `tests-bypasses.mjs` — um hook sem testes bloqueia trabalho legitimo em silencio | **So-Claude Code.** A verificacao equivalente vive em `.agent/scripts/check-*.mjs`, que qualquer agente corre |
